...
SOCKET CSocks::Connect()
{
    ...
    std::cout <<"soc socks5: " << hSocketSock << std::endl;
    struct timeval timeout;
    timeout.tv_sec = 2;
    timeout.tv_usec = 0;

    if (setsockopt (hSocketSock, SOL_SOCKET, SO_RCVTIMEO, (char *)&timeout,sizeof(timeout)) < 0)
        std::cout << "setsockopt failed\n";

    if (setsockopt (hSocketSock, SOL_SOCKET, SO_SNDTIMEO, (char *)&timeout,sizeof(timeout)) < 0)
        std::cout << "setsockopt failed\n";

    int iResult = ioctlsocket(hSocketSock, FIONBIO, &iMode);
